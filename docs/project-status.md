...
The shared Research Core now has an explicit semantic specialization axis:

- `GENERAL_RESEARCH` (default): domain-neutral, question-conditioned research.
- `CUSTOMER_INSIGHT`: opt-in customer/user-research specialization for stated vs latent needs, JTBD and legacy commercial projection fields.

ReasoningProfile is independent from AnalysisMode, ResearchStage, ExecutionMode and ExecutionProfile. It never changes evidence identity. A profile change is recorded as an execution/semantic configuration change, so run comparison does not misclassify it as new evidence.

Both profiles reuse the same grounding, competing-hypothesis, counter-evidence, causal/identification, ResearchGap and append-only iteration semantics. Source type, namespace and document wording never select a profile implicitly.
