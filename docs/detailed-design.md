...
| ビルド種別 | 用途 | デモデータ |
|---|---|---|
| デモビルド（`make build-demo`） | 商談・営業デモ | 架空の政策調査データを埋め込み。`--demo` 起動可 |
...
架空の政策調査データ（処置群と比較群がともに増加し、事前トレンド・外部文脈・測定変更を含む）に対し、相関から政策効果を断定せず、複数仮説・反証・不足Evidence・`NOT_IDENTIFIED`を保つことを回帰評価する。
