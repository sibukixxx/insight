...
func BuildLongitudinalTimeline(run domain.ResearchRun, src TimelineSources) domain.LongitudinalTimeline {
	...
	for i, it := range its {
		...
		attribution := timelineAttribution(i == 0 || len(events) > 0, instrument != nil && instrument.Execution == domain.FingerprintChanged)
...
func timelineAttribution(evidence, instrument bool) domain.ChangeAttribution {
	switch {
