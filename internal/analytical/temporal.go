...
func temporalProjection(a Artifact, m MetricDefinition, r Result, index int) (*TemporalEvidence, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("snapshot temporal evidence: %w", err)
	}
...
func periodBounds(p Period) (time.Time, time.Time, int, error) {
	...
	case "calendar-day":
		layout = "2006-01-02"
	default:
		return time.Time{}, time.Time{}, 0, fmt.Errorf("unsupported period basis %q", p.Basis)
	}
	s, e := time.Time{}, time.Time{}
	var err error
	if s, err = time.Parse(layout, p.Start); err != nil {
		return s, e, 0, err
	}
	if e, err = time.Parse(layout, p.End); err != nil {
		return s, e, 0, err
	}
	if e.Before(s) {
		return s, e, 0, fmt.Errorf("reversed period")
	}
	var count int
	switch p.Basis {
	case "calendar-year":
		count = e.Year() - s.Year() + 1
		e = e.AddDate(1, 0, 0)
	case "calendar-month":
		count = (e.Year()-s.Year())*12 + int(e.Month()) - int(s.Month()) + 1
		e = e.AddDate(0, 1, 0)
	case "calendar-day":
		count = int(e.Sub(s).Hours()/24) + 1
		e = e.AddDate(0, 0, 1)
	}
...
func validateTemporalResult(r Result, metrics []MetricDefinition, enclosing Period) error {
	...
	if t.ObservedAt.IsZero() || blank(t.Geography) {
		return fmt.Errorf("temporal observedAt and geography are required")
	}
	if t.Origin != "observed" && t.Origin != "derived" {
		return fmt.Errorf("origin must be observed or derived")
	}
	...
	for _, m := range metrics {
		if m.ID == r.MetricID && blank(m.Version) {
			return fmt.Errorf("temporal metric version is required")
		}
	}
	p := r.Period
	if p.Basis == "" {
		p.Basis = enclosing.Basis
	}
	...
	if s.Before(as) || e.After(ae) {
		return fmt.Errorf("result period outside artifact period")
	}
	return nil
}

func number(r Result) (float64, bool) {
	if r.Missing || !json.Valid(r.Value) || len(r.Value) == 0 || bytes.Equal(bytes.TrimSpace(r.Value), []byte("null")) {
		return 0, false
	}
	var v any
	d := json.NewDecoder(bytes.NewReader(r.Value))
	d.UseNumber()
	if d.Decode(&v) != nil {
		return 0, false
	}
...
func sortedSources(p *TemporalEvidence) []string {
	...
	for _, d := range p.Datasets {
		b, _ := json.Marshal(d)
		sources = append(sources, "dataset:"+string(b))
	}
	for _, s := range p.Provenance {
		b, _ := json.Marshal(s)
		sources = append(sources, "source:"+string(b))
	}
...
func CompareObservations(previous, current domain.Observation) ObservationDelta {
	...
	for _, v := range []*TemporalEvidence{p, c} {
		if err := validateTemporalProvenance(v); err != nil {
			warn("INVALID_PROVENANCE", err.Error())
		}
	}
	definition := func(name string, changed bool) {
		if changed {
			d.DataDefinitionChanges = append(d.DataDefinitionChanges, name)
			warn("INCOMPATIBLE_"+strings.ToUpper(name), "Changed "+name+" cannot be interpreted as real-world change.")
		}
	}
	...
	for k := range p.Result.Dimensions {
		keys[k] = true
	}
	for k := range c.Result.Dimensions {
		keys[k] = true
	}
	for k := range keys {
		pv, pok := p.Result.Dimensions[k]
		cv, cok := c.Result.Dimensions[k]
		if pok != cok || pv != cv {
			d.ChangedDimensions = append(d.ChangedDimensions, k)
		}
	}
	...
	pv, pok := number(p.Result)
	cv, cok := number(c.Result)
	if !pok || !cok {
	...
	if math.IsInf(absolute, 0) || math.IsNaN(absolute) {
		warn("NON_FINITE_DELTA", "The numeric delta cannot be represented.")
		return d
	}
	d.Valid = true
	d.Absolute = &absolute
	...
	if d.SourceChanges {
		warn("SOURCE_CHANGED", "Source or dataset provenance changed; verify comparability.")
	}
...
func CompareObservationSeries(observations []domain.Observation) []ObservationDelta {
	...
	for i := 1; i < len(observations); i++ {
		out = append(out, CompareObservations(observations[i-1], observations[i]))
	}
	return out
}
...
func ToCandidatesForAnalysis(artifact Artifact, analysisID string) ([]Candidate, error) {
	...
	for i := range candidates {
		...
		c.Observation.ID = id
		c.Observation.AnalysisID = analysisID
		c.Evidence.ID = id
		c.Evidence.ObservationID = &c.Observation.ID
	}
...
func validateTemporalProvenance(p *TemporalEvidence) error {
	...
	if blank(p.ArtifactID) || p.ResultIndex < 0 || p.GeneratedAt.IsZero() {
		return fmt.Errorf("artifact identity, result index and generation time are required")
	}
	if err := validateHash("artifactHash", p.ArtifactHash); err != nil {
		return err
	}
	if blank(p.Spec.Kind) || blank(p.Spec.Reference) {
		return fmt.Errorf("query/spec reference is required")
	}
	if err := validateHash("spec.hash", p.Spec.Hash); err != nil {
		return err
	}
	if len(p.Datasets) == 0 || len(p.Provenance) == 0 {
		return fmt.Errorf("datasets and source provenance are required")
	}
	ids := map[string]bool{}
	for _, d := range p.Datasets {
		if blank(d.ID) || blank(d.Version) || ids[d.ID] {
			return fmt.Errorf("invalid dataset identity")
		}
		if err := validateHash("dataset.hash", d.Hash); err != nil {
			return err
		}
		ids[d.ID] = true
	}
	...
	for _, source := range p.Provenance {
		if !ids[source.DatasetID] || blank(source.Source) || source.RetrievedAt.IsZero() {
	...
	for id := range ids {
		if !covered[id] {
			return fmt.Errorf("dataset has no source provenance")
		}
	}
	return nil
}
