...
func TestTemporalJapanEUThreeWindows(t *testing.T) {
	...
	for i, d := range ds {
		...
		if d.Previous.AnalysisID != "analysis-1" || d.Current.AnalysisID != "analysis-1" {
			t.Fatal("lost run ownership")
		}
		if d.SourceChanges || len(d.ChangedDimensions) > 0 || len(d.DataDefinitionChanges) > 0 {
			t.Fatal("false definition change")
		}
	...
	if !hasWarning(ds[1], "QUALITY_FLAGS_PRESENT") || len(ds[1].Comparison.Result.Quality) != 1 {
		t.Fatal("lost provisional quality")
	}
...
func TestTemporalIncompatibilitiesSuppressArithmetic(t *testing.T) {
	tests := []struct {
		...
		{"overlap", "INCOMPATIBLE_PERIOD", func(o *domain.Observation) {
			o.Temporal.Result.Period = Period{Start: "2022", End: "2023", Basis: "calendar-year"}
		}},
		...
		{"query", "INCOMPATIBLE_SPEC", func(o *domain.Observation) {
			o.Temporal.Spec.Hash.Value = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
		}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if d.Valid || d.Absolute != nil || d.Relative != nil || d.Direction != "unknown" || !hasWarning(d, tt.warning) {
				t.Fatalf("%+v", d)
			}
		})
	}
...
func TestMissingIsNotZero(t *testing.T) {
	for _, raw := range []string{"", "null", "\"0\"", "true"} {
		obs := temporalObservations(t)
		obs[1].Temporal.Result.Value = json.RawMessage(raw)
		d := CompareObservations(obs[0], obs[1])
		if d.Valid || d.Absolute != nil {
			t.Fatalf("non-number %q: %+v", raw, d)
		}
	}
	obs := temporalObservations(t)
	obs[1].Temporal.Result.Missing = true
	if d := CompareObservations(obs[0], obs[1]); d.Valid || d.Absolute != nil {
		t.Fatal("missing coerced to value")
	}
	obs = temporalObservations(t)
	obs[0].Temporal.Result.Value = json.RawMessage("0")
	d := CompareObservations(obs[0], obs[1])
	if !d.Valid || d.Absolute == nil || *d.Absolute != 115 || d.Relative != nil || !hasWarning(d, "ZERO_BASELINE") {
		t.Fatalf("%+v", d)
	}
	obs = temporalObservations(t)
	obs[0].Temporal.Result.Value = json.RawMessage("-100")
	d = CompareObservations(obs[0], obs[1])
	if !d.Valid || *d.Relative != 2.15 {
		t.Fatalf("negative baseline: %+v", d)
	}
}
...
func TestTemporalSourceChangeAndSnapshotIsolation(t *testing.T) {
	...
	if cs[0].Observation.Temporal.Datasets[0].Version == "modified" || cs[0].Observation.Temporal.Result.Dimensions["destination"] == "US" {
		t.Fatal("mutable artifact leaked")
	}
	obs := temporalObservations(t)
	obs[1].Temporal.Datasets[0].Version = "revised"
	d := CompareObservations(obs[0], obs[1])
	if !d.Valid || !d.SourceChanges || !hasWarning(d, "SOURCE_CHANGED") {
		t.Fatalf("%+v", d)
	}
	if d.Baseline.Datasets[0].Version == d.Comparison.Datasets[0].Version {
		t.Fatal("old provenance overwritten")
	}
}
...
func TestTemporalLegacyCompatibilityAndRunIDs(t *testing.T) {
	...
	if cs[0].Observation.Temporal != nil || cs[0].Evidence.Temporal != nil {
		t.Fatal("invented legacy temporal metadata")
	}
	d := CompareObservations(cs[0].Observation, cs[1].Observation)
	if d.Valid || !hasWarning(d, "MISSING_TEMPORAL_EVIDENCE") {
		t.Fatalf("%+v", d)
	}
	encoded, _ := json.Marshal(cs[0].Observation)
	var legacy map[string]any
	_ = json.Unmarshal(encoded, &legacy)
	if _, ok := legacy["temporal"]; ok {
	...
	if first[0].Observation.ID == second[0].Observation.ID || second[0].Observation.AnalysisID != "second" {
		t.Fatal("run identity collision")
	}
	if *second[0].Evidence.ObservationID != second[0].Observation.ID || second[0].Evidence.Type != domain.EvidenceNeutral {
		t.Fatal("evidence link or neutrality lost")
	}
	if _, err := ToCandidatesForAnalysis(a, ""); err == nil {
		t.Fatal("unowned run accepted")
	}
	if len(CompareObservationSeries(nil)) != 0 {
...
func TestTemporalImportValidation(t *testing.T) {
	...
	if !reflect.DeepEqual(a, *got) {
		t.Fatal("temporal round-trip differs")
	}
}
...
func TestTemporalDirectionAndCalendarMonths(t *testing.T) {
	...
	d := CompareObservations(obs[0], obs[1])
	if !d.Valid || d.Direction != "decrease" || *d.Absolute != -10 {
		t.Fatalf("%+v", d)
	}
	obs[1].Temporal.Result.Value = json.RawMessage("100")
	if d := CompareObservations(obs[0], obs[1]); !d.Valid || d.Direction != "unchanged" {
		t.Fatalf("%+v", d)
	}
	obs[1].Temporal.Result.Value = json.RawMessage("1e400")
	if d := CompareObservations(obs[0], obs[1]); d.Valid {
...
func TestTemporalSnapshotRejectsUnserializableProvenance(t *testing.T) {
	...
	if _, err := ToCandidatesForAnalysis(a, "analysis"); err == nil {
		t.Fatal("snapshot silently discarded provenance")
	}
}
