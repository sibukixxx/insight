...
func TestImportFixtureAndUnknownFieldTolerance(t *testing.T) {
	a := fixture(t)
	if a.ArtifactSchema != Schema || len(a.Results) != 3 {
	...
	if _, err := Import(data); err != nil {
		t.Fatalf("additive field should be tolerated: %v", err)
	}
}
...
func TestAdapterProducesOnlyNeutralCandidates(t *testing.T) {
	...
	if len(candidates) != len(a.Results) {
		t.Fatalf("got %d candidates", len(candidates))
	}
	for _, candidate := range candidates {
		if candidate.Evidence.Type != domain.EvidenceNeutral {
			t.Fatalf("analytical result promoted to %q", candidate.Evidence.Type)
		}
		if candidate.Evidence.InsightID != "" {
			t.Fatal("candidate must not be attached to an Insight")
		}
		if candidate.Observation.Behavior == "" {
			t.Fatal("candidate boundary must remain explicit")
		}
	}
}
...
func TestReproducibilityKeyIgnoresDatasetOrder(t *testing.T) {
	...
	if a.ReproducibilityKey() != b.ReproducibilityKey() {
		t.Fatal("key depends on dataset order")
	}
}
