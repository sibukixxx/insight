...
func (a Artifact) Validate() error {
	...
	if err := validateHash("artifactHash", a.ArtifactHash); err != nil { return fail("%v", err) }
	if len(a.Datasets) == 0 { return fail("at least one dataset is required") }
	datasetIDs := map[string]struct{}{}
	for i, dataset := range a.Datasets {
		if blank(dataset.ID) || blank(dataset.Version) { return fail("datasets[%d] requires id and version", i) }
		if _, exists := datasetIDs[dataset.ID]; exists { return fail("duplicate dataset id %q", dataset.ID) }
		datasetIDs[dataset.ID] = struct{}{}
		if err := validateHash(fmt.Sprintf("datasets[%d].hash", i), dataset.Hash); err != nil { return fail("%v", err) }
	}
	if blank(a.Spec.Kind) || blank(a.Spec.Reference) { return fail("spec kind and reference are required") }
	if err := validateHash("spec.hash", a.Spec.Hash); err != nil { return fail("%v", err) }
	if err := validatePeriod("period", a.Period); err != nil { return fail("%v", err) }
	if blank(a.Population.Description) { return fail("population.description is required") }
	if len(a.Metrics) == 0 || len(a.Results) == 0 { return fail("metrics and results must not be empty") }
	metricIDs := map[string]struct{}{}
	for i, metric := range a.Metrics {
		if blank(metric.ID) || blank(metric.Name) || blank(metric.Unit) { return fail("metrics[%d] requires id, name and unit", i) }
		if _, exists := metricIDs[metric.ID]; exists { return fail("duplicate metric id %q", metric.ID) }
		metricIDs[metric.ID] = struct{}{}
	}
	for i, result := range a.Results {
		if _, exists := metricIDs[result.MetricID]; !exists { return fail("results[%d] references unknown metric %q", i, result.MetricID) }
		if err := validatePeriod(fmt.Sprintf("results[%d].period", i), result.Period); err != nil { return fail("%v", err) }
		if result.Missing && len(result.Value) != 0 { return fail("results[%d] cannot have value when missing", i) }
		if !result.Missing && !validScalar(result.Value) { return fail("results[%d].value must be a JSON scalar", i) }
	}
	...
	if len(a.Provenance) == 0 { return fail("source provenance is required") }
	covered := map[string]struct{}{}
	for i, provenance := range a.Provenance {
		if _, exists := datasetIDs[provenance.DatasetID]; !exists { return fail("provenance[%d] references unknown dataset %q", i, provenance.DatasetID) }
		if blank(provenance.Source) || provenance.RetrievedAt.IsZero() { return fail("provenance[%d] requires source and retrievedAt", i) }
		covered[provenance.DatasetID] = struct{}{}
	}
	for datasetID := range datasetIDs {
		if _, ok := covered[datasetID]; !ok { return fail("dataset %q has no source provenance", datasetID) }
	}
	return nil
}
...
func validatePeriod(name string, p Period) error {
	if blank(p.Start) || blank(p.End) { return fmt.Errorf("%s requires start and end", name) }
	if p.Start > p.End { return fmt.Errorf("%s.start must not be after end", name) }
	return nil
}
...
func validScalar(raw json.RawMessage) bool {
	...
	if decoder.Decode(&v) != nil { return false }
	switch v.(type) { case nil, bool, string, json.Number: return true }
	return false
}
...
func ToCandidates(artifact Artifact) ([]Candidate, error) {
	if err := artifact.Validate(); err != nil { return nil, err }
	metrics := make(map[string]MetricDefinition, len(artifact.Metrics))
	for _, metric := range artifact.Metrics { metrics[metric.ID] = metric }
...
func resultStatement(metric MetricDefinition, result Result) string {
	value := "missing"
	if !result.Missing { value = string(result.Value) }
	dimensions := make([]string, 0, len(result.Dimensions))
	for key, val := range result.Dimensions { dimensions = append(dimensions, key+"="+val) }
	sort.Strings(dimensions)
	suffix := ""
	if len(dimensions) > 0 { suffix = " (" + strings.Join(dimensions, ", ") + ")" }
